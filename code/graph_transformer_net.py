...
def get_adjacent_indices_for_dgl(conv_output_shape):
    ...
    """
    h, w, d = conv_output_shape  # 输出的高度、宽度、深度
    total_elements = h * w * d  # 总的元素个数
    
    # 生成展平的索引 (h * w * d) 的矩阵
    indices = np.arange(total_elements).reshape(h, w, d)
    ...
    for i in range(h):
        for j in range(w):
            for k in range(d):
                current_idx = indices[i, j, k]
                
                # 检查每个方向的相邻点，确保不越界
                
                # y方向相邻
                if i > 0:
                    neighbor_idx = indices[i - 1, j, k]  # 上面
                    src_list.append(current_idx)
                    dst_list.append(neighbor_idx)
                
                if i < h - 1:
                    neighbor_idx = indices[i + 1, j, k]  # 下面
                    src_list.append(current_idx)
                    dst_list.append(neighbor_idx)
                
                # x方向相邻
                if j > 0:
                    neighbor_idx = indices[i, j - 1, k]  # 左边
                    src_list.append(current_idx)
                    dst_list.append(neighbor_idx)
                
                if j < w - 1:
                    neighbor_idx = indices[i, j + 1, k]  # 右边
                    src_list.append(current_idx)
                    dst_list.append(neighbor_idx)
                
                # z方向相邻
                if k > 0:
                    neighbor_idx = indices[i, j, k - 1]  # 前面
                    src_list.append(current_idx)
                    dst_list.append(neighbor_idx)
                
                if k < d - 1:
                    neighbor_idx = indices[i, j, k + 1]  # 后面
                    src_list.append(current_idx)
                    dst_list.append(neighbor_idx)
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=64, patch_size=16, in_c=1, embed_dim=768, norm_layer=None):
        super().__init__()
        ...
        self.proj = nn.Conv3d(in_c, embed_dim, kernel_size=patch_size, stride=patch_size, padding=0)
...
class GraphTransformerNet(nn.Module):
    def __init__(self):
        ...
        self.patch_embed = PatchEmbed(img_size=64, patch_size=16, in_c=1, embed_dim=768)
        self.patch_embed_mask = PatchEmbed(img_size=64, patch_size=16, in_c=1, embed_dim=768)
        self.src_list, self.dst_list = get_adjacent_indices_for_dgl((4,4,4))
        self.MLP_layer_0 = nn.Linear(66,1)
        # self.MLP_layer_0 = nn.Linear(128,1)
        self.n_classes=2
        config = MambaConfig(d_model=122, n_layers=2)
        self.mamba = Mamba(config)
        # self.pooling = nn.AdaptiveAvgPool2d((1, 768))
        self.epooling =  nn.AdaptiveAvgPool2d((1, 768))
        self.hcpooling =  nn.AdaptiveAvgPool2d((1, 122))
        ...
        # )
        self.pos_embed = nn.Parameter(torch.zeros(1, 64, 768))
        self.pos_embed_m = nn.Parameter(torch.zeros(1, 64, 768))
    ...
    def forward(self, images, masks, clinical_datas):        
        ...
        # h=h+hm
        # hc = self.proj_clinical(clinical_datas.unsqueeze(1).unsqueeze(2)).squeeze(1) #.expand(-1, 64, -1)
        ...
        # for conv in self.layers:
            ...
            # print(hc.shape)
            hc = sml(hc,hc,hc)
            hc = cml(pproj(h.view(-1,64,768)),hc,hc)

        # # output
        # h=self.MLP_layer_0(h.view(-1,64,768).transpose(1, 2)).squeeze(-1)
        # h_out = self.MLP_layer(h)
        e=self.pooling_proj_e(self.epooling(e.view(-1,288,768)))
        hc=self.hcpooling(hc)
        h=self.pooling_proj(h.view(-1,64,768))
