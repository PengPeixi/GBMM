...
def load_fold_predictions(fold_number, epoch_number):
    file_path = f"/root/autodl-tmp/v1/3d/code/backup/results1/fold_{fold_number}_epoch_{epoch_number}_test_predictions.csv"
...
def load_last_three_epochs_predictions(num_folds, num_epochs):
    ...
    for epoch in range(num_epochs - 9, num_epochs + 1):
    # for epoch in range(num_epochs, num_epochs+1):  # 这里是加载最后三代
        # for fold in range(1, num_folds + 1):  # 加载三折
        for fold in range(1, 2):
        # for fold in [1,3,6,8]:  # 加载三折
...
if __name__ == '__main__':
    ...
    main_optimization(num_folds=8, num_epochs=40)  # 运行主程序时保证三折
    # for j in range(1,9):
    #     print('*************************************************************************')
    #     print('fold: ',j)
    #     for i in range(1,9):
