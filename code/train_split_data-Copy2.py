...
def main(args):
    ...
    skf = StratifiedKFold(n_splits=2, shuffle=True, random_state=110)
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(images_path, images_label)):
        ...
        for epoch in range(args.epochs):
            ...
            if val_auc >= best_val_metric:
                ...
                torch.save(model.state_dict(), f"./weights/val_model_fold_{fold + 1}_epoch_{epoch + 1}.pth")
                
            if test_auc >= best_metric:
                best_metric = test_auc
                best_metric_epoch = epoch + 1
                torch.save(model.state_dict(), f"./weights/test_model_fold_{fold + 1}_epoch_{epoch + 1}.pth")
