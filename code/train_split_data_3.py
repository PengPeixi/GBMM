...
def main(args):
    ...
    if not os.path.exists("./weights/teacher"):
        os.makedirs("./weights/teacher")
    if not os.path.exists("./results/teacher"):
    ...
    skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=611)
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(images_path, images_label)):
        ...
        train_dataset_degrad = MyDataSet(images_path=train_images_path,
                                  masks_path=train_masks_path,
                                  clinical_data=train_clinical_data,
                                  images_class=train_images_label,
                                  image_transform=image_transform["train"], mask_transform=mask_transform["train"],apply_degradation=True)
        ...
        val_dataset_degrad = MyDataSet(images_path=val_images_path,
                                masks_path=val_masks_path,
                                clinical_data=val_clinical_data,
                                images_class=val_images_label,
                                image_transform=image_transform["val"], mask_transform=mask_transform["val"],apply_degradation=True)
        ...
        train_loader_degrad = torch.utils.data.DataLoader(train_dataset_degrad,
                                                   batch_size=batch_size,
                                                   shuffle=True,
                                                   pin_memory=True,
                                                   num_workers=nw,
        ...
        val_loader_degrad = torch.utils.data.DataLoader(val_dataset_degrad,
                                                 batch_size=batch_size,
                                                 shuffle=False,
                                                 pin_memory=True,
                                                 num_workers=nw,
        ...
        model.load_state_dict(torch.load(f"./weights/model_fold_{fold + 1}_best_epoch.pth"))
        
        teacher = GraphTransformerNet().to(device)
        teacher.load_state_dict(torch.load(f"./weights/model_fold_{fold + 1}_best_epoch.pth"))
        ...
        momentum_schedule = cosine_scheduler(0.99, 1,
                                               40, 2*len(train_dataset))
        ...
        for epoch in range(args.epochs):
            ...
            train_loss, train_acc, train_auc = train_one_epoch(model=model,
                                                               ...
                                                               data_loader_2=train_loader_degrad,
                                                               device=device,
                                                               epoch=epoch,
                                                               teacher=teacher,it=it,momentum_schedule=momentum_schedule)

            scheduler.step()

            # Validate
            val_loss, val_acc, val_auc, _, _, _ = evaluate(model=teacher,
                                                           data_loader=val_loader,
                                                           device=device,
                                                           epoch=epoch)
            val_loss_degrad, val_acc_degrad, val_auc_degrad,  _,_,_ = evaluate(model=teacher,
                                         data_loader=val_loader,
                                         device=device,
                                         epoch=epoch,
                                         data_loader_2=train_loader_degrad,
            ...
            test_loss, test_acc, test_auc, l0, l1, gt = evaluate(model=teacher,
            ...
            csv_file_path = f"./results/teacher/fold_{fold + 1}_epoch_{epoch + 1}_test_predictions.csv"
            save_predictions_to_csv(predictions, csv_file_path)

            torch.save(teacher.state_dict(), f"./weights/teacher/model_fold_{fold + 1}_epoch_{epoch + 1}.pth")
            ...
            tags = ["train_loss", "train_acc", "train_auc", "val_loss", "val_acc", "val_auc", "learning_rate",
                    "test_loss", "test_acc", "test_auc","val_loss_degrad","val_acc_degrad","val_auc_degrad"]
            ...
            tb_writer.add_scalar(tags[10], val_loss_degrad, epoch)
            tb_writer.add_scalar(tags[11], val_acc_degrad, epoch)
            tb_writer.add_scalar(tags[12], val_auc_degrad, epoch)
            
            if test_auc >= best_metric:
                best_metric = test_auc
                best_metric_epoch = epoch + 1
                # csv_file_path = f"./results/fold_{fold + 1}_test_predictions.csv"
                # save_predictions_to_csv(predictions, csv_file_path)
                # torch.save(model.state_dict(), f"./weights/model_fold_{fold + 1}_best_epoch.pth")
